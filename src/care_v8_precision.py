"""
CARE v8: Precision-Focused Detection
=====================================
Critical insight from v7: Coverage (F_0.5) counts ALL detections BEFORE
the event start as FALSE POSITIVES. With β=0.5, precision is weighted
MORE than recall. So we MUST be SILENT during the first ~170 days and
fire ONLY during the event window (last 6-23 days).

Strategy: Use power-binned z-scores but with EXPONENTIAL ACCUMULATION
that only fires after sustained, escalating anomaly. The CUSUM must
reach threshold LATE, not early.

Also: Fix id column (was wrongly looking for row_id).
"""
...
TARGETS = [
    ...
    "sensor_47_avg", "sensor_58_avg", "sensor_109_avg",
    "sensor_44_avg", "sensor_87_avg", "sensor_75_avg",
    "sensor_167_avg",
]
...
print("="*100)
print("CARE v8: Precision-Focused Binned Detection (20-loop sweep)")
print("="*100)

best_care=-1; best_results=None; best_params={}

configs = [
    # (n_bins, z_thresh, min_ch, decay, cusum_h)
    # decay: CUSUM decrement rate. Higher = alarms die faster = more precision
    (20, 3.0, 2, 3, None),    # No CUSUM, rolling+voting
    (20, 3.5, 2, 3, None),
    (20, 4.0, 2, 3, None),
    (20, 4.0, 3, 3, None),
    (20, 4.5, 3, 3, None),
...
for loop, (n_bins, z_thresh, min_ch, decay, cusum_h) in enumerate(configs):
    results = []
    ...
    for eid in all_ids:
        ...
        for s in avail:
            ...
            # Rolling mean z-score (3-day window)
    ...
    print(f"  [{loop+1:2d}] z={z_thresh} ch={min_ch} → TP={tpe}/27 FP={fpe}/31 C={mcov:.4f} A={macc:.4f} R={rel:.4f} E={mws:.4f} CARE={care:.4f}{marker}")
    
    if care>best_care:
        best_care=care; best_params={"z":z_thresh,"ch":min_ch,"loop":loop+1}
...
if best_results is not None:
    ...
    # Detailed per-event for anomalies
    print(f"\n{'ID':>4} {'D':>2} {'Crit':>6} {'Cov':>7} {'WS':>6} {'Lead':>6} {'TP':>5} {'FP':>6} {'FN':>5} {'Ch':>3} {'A%':>5}")
    for _,r in ba.iterrows():
        d="Y" if r['det'] else "N"; ld_s=f"{r['ld']:.0f}" if pd.notna(r.get('ld')) else "-"
        print(f"  {int(r['eid']):>4} {d:>2} {r['mc']:>6.0f} {r['cov']:>7.4f} {r['ws']:>6.3f} {ld_s:>6} {r['tp']:>5} {r['fp']:>6} {r['fn']:>5} {r['nch']:>3} {r['ap']:>5.1f}")
    
    print(f"\nNormal: FP events={bn['fa'].sum()}/31")
    for _,r in bn[bn['fa']].iterrows():
        print(f"  Event {int(r['eid'])}: crit={r['mc']:.0f}")
    
    best_results.to_csv(r"C:\Users\nijat\OneDrive\Documents\WIND FIN\WindFarmC-Project\results\care_v8_best.csv", index=False)
